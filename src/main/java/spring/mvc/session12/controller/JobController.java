...
//		return "session12/job";
	...
	@PostMapping(value = "/")
	public String add(Model model,@Valid @ModelAttribute Job job, BindingResult result) {
		if(result.hasErrors()) {
			model.addAttribute("_method","POST");
			model.addAttribute("jobs",jobDao.query());
			model.addAttribute("employees",employeeDao.query());
	...
	@PutMapping(value = "/")
	public String update(@Valid @ModelAttribute Job job , BindingResult result, Model model) {
		if(result.hasErrors()) {
			model.addAttribute("_method","POST");
			model.addAttribute("jobs",jobDao.query());
			model.addAttribute("employees",employeeDao.query());
			model.addAttribute("pageCount", getPageCount());
			//model.addAttribute("Job",job); 因為參數有設定 @ModelAttribute 所以系統會自動帶入 就不用手動撰寫帶入 job
			return "session12/job";
		}
		jobDao.update(job);
		return "redirect:./";
